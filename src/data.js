...
function initialize_maps(config) {
    ...
    if (config['save_expected_map_dist']) {
        ...
        fs.writeFileSync(
            './data/current_map_dist.json',
            JSON.stringify(current_dist, null, 2)
        );
    }
    ...
    for (let map of maps) {
        for (let mode in map.mapvote_weights) {
            if (mode in mode_probs)
                mode_probs[mode][map.name] = map.mapvote_weights[mode];
    ...
    for (let mode in mode_probs) {
        for (let map of maps) {
            ...
            if (
                Object.keys(mode_probs[mode]).includes(map.name) &&
                map.cluster_overlap > 0 &&
                config['use_lock_time_modifier']
            ) {
...
function get_dist(maps) {
    const weight_params = JSON.parse(
        fs.readFileSync('./data/weight_params.json')
    );
    let current_map_dist = {};
    for (let map of maps) {
        current_map_dist[map.name] = {};
        for (let mode in map.layers) {
            current_map_dist[map.name][mode] =
                map.mapvote_weights[mode] / map.mapvote_weight_sum[mode];
        }
    }
...
function normalize_mapvote_weights(maps, save_expected_map_dist = false) {
    ...
    if (save_expected_map_dist) {
        ...
        fs.writeFileSync(
            './data/current_map_dist.json',
            JSON.stringify(map_dist, null, 2)
        );
    }
}
...
class Map {
     ...
     */
    lock_layer(layer) {
        this.locked_layers.push({
            locktime: this.layer_locktime,
            layer: layer,
        });
        ...
        if (index > -1) {
            ...
            console.log(
                `WARNING: Layer ${layer.name} not found in Map ${this.name}`
            );
        }
    }
    /**
    ...
    add_mapvote_weights(mode) {
        ...
        if (this.layers.length == 0) {
            console.log(
                `No layers added to map ${this.name}, could not calculate mapvote_weights!`
            );
            return;
        }
        let votesum = {};
        ...
        for (let mode of Object.keys(votesum)) {
            means[mode] =
                (1 / votesum[mode].length) * utils.sumArr(votesum[mode]);
            for (let v of votesum[mode]) {
                if (weights[mode])
                    weights[mode].push(Math.exp(-Math.pow(means[mode] - v, 2)));
            ...
            temp[mode] = utils.sigmoid(
                utils.sumArr(weights[mode]),
                slope,
                shift
            );
        }
    ...
    calculate_vote_weights_by_mode(mode) {
        ...
        for (let mode of modes) {
            let votes = [];
            for (let layer of this.layers[mode]) votes.push(layer.votes);
            let weights = utils.normalize(
                utils.sigmoidArr(votes, sigmoid_slope, sigmoid_shift)
            );
...
function get_mode_to_pools_dict(config) {
    ...
    for (let mode_group of Object.keys(config['mode_distribution']['pools'])) {
        if (mode_group != null) {
            for (let mode of Object.keys(
                config['mode_distribution']['pools'][mode_group]
            )) {
...
function fix_unavailables() {
    ...
    fs.writeFileSync(
        './data/mode_distribution_overwrite.json',
        JSON.stringify(config['mode_distribution'], null, 2)
    );
    ...
    av_maps.sort();
    fs.writeFileSync(
        './data/maps_overwrite.json',
        JSON.stringify(av_maps, null, 2)
    );
}
/**
...
function build_config(warning = false) {
    let config = JSON.parse(fs.readFileSync('./config.json'));
    let maps_overwrite = JSON.parse(
        fs.readFileSync('./data/maps_overwrite.json')
    );
    ...
    let mode_distribution_overwrite = JSON.parse(
        fs.readFileSync('./data/mode_distribution_overwrite.json')
    );
    if (
        JSON.stringify(mode_distribution_overwrite) !=
        JSON.stringify(config['mode_distribution'])
    ) {
...
function check_changes() {
    ...
    check['bioms'] = crypto
        .createHash('md5')
        .update(
            JSON.stringify(JSON.parse(fs.readFileSync('./data/bioms.json')))
        )
        .digest('hex');
    let c_config = {};
    ...
    check['config'] = crypto
        .createHash('md5')
        .update(JSON.stringify(c_config))
        .digest('hex');

    let maps = JSON.parse(fs.readFileSync('./data/maps_overwrite.json'));
    maps.sort();
    check['maps_overwrite'] = crypto
        .createHash('md5')
        .update(JSON.stringify(maps))
        .digest('hex');

    let modes = JSON.parse(
        fs.readFileSync('./data/mode_distribution_overwrite.json')
    );
    check['modes_overwrite'] = crypto
        .createHash('md5')
        .update(JSON.stringify(modes))
        .digest('hex');

    if (
        crypto.createHash('md5').update(JSON.stringify(save)).digest('hex') !=
        crypto.createHash('md5').update(JSON.stringify(check)).digest('hex')
    ) {
